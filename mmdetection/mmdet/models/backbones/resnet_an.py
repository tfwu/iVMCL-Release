...
from mmcv.ops import ExtractHOG
from mmcv.runner import load_checkpoint
from torch.nn.modules.batchnorm import _BatchNorm

from mmdet.utils import get_root_logger
from ..builder import BACKBONES
from ..utils import ResLayerAN
...
class BasicBlock(nn.Module):
    ...
    def __init__(self,
        ...
        if plugins is not None:
            allowed_position = ['after_conv1', 'after_bn1',
                                'after_conv2']
...
class Bottleneck(nn.Module):
    ...
    def __init__(self,
        ...
        if plugins is not None:
            allowed_position = ['after_conv1', 'after_bn1',
                                'after_conv2', 'after_bn2', 'after_conv3']
            assert all(p['position'] in allowed_position for p in plugins)
        ...
        self.norm3_name, norm3 = build_norm_layer(
            norm_cfg3, planes * self.expansion, postfix=3)
...
class ResNetAN(nn.Module):
    """ResNet backbone.
    Add the support of AttentiveNorm https://arxiv.org/abs/1908.01259
    ...
    Args:
        ...
        norm_cfg_ds (dict): Dictionary to construct and config norm layer
            for downsampling op.
        norm_cfg_stem (dict): Dictionary to construct and config norm layer
            for the stem layer.
        norm_cfg1 (dict): Dictionary to construct and config norm layer
            for the 1st conv op in a block (Basic or Bottleneck).
        norm_cfg2 (dict): Dictionary to construct and config norm layer
            for the 2nd conv op in a block (Basic or Bottleneck).
        norm_cfg3 (dict): Dictionary to construct and config norm layer
            for the 3rd conv op in a block (Bottleneck).
        num_affine_trans (list): the number of affine transformation components
            in the attention normalization (AN) at each stage
        ...
        num_classes (int): Use the model for classification task if > 0

    Example:
        >>> from mmdet.models import ResNet
        >>> import torch
        >>> self = ResNetAN(depth=18)
        >>> self.eval()
    ...
    def __init__(self,
                 ...
                 num_classes=0,
                 with_hog=False,
                 stage_with_hog=(False, False, False, False, False)):
        """
        ...
        self.with_hog = with_hog
        self.stage_with_hog = stage_with_hog
        if with_hog:
            assert len(stage_with_hog) == num_stages + 1

        stem_in_channels = in_channels

        if with_hog and stage_with_hog[0]:
            norm_cfg_ = self.norm_cfg_stem.copy()
            norm_cfg_['affine'] = False
            self.hog0 = nn.Sequential(
                ExtractHOG(2),
                build_norm_layer(norm_cfg_, 31)[1]
            )
            stem_in_channels += 31

        if 'Attn' in self.norm_cfg_stem['type']:
            self.norm_cfg_stem['num_affine_trans'] = self.num_affine_trans[0]
        self._make_stem_layer(stem_in_channels, stem_channels)
        ...
        for i, num_blocks in enumerate(self.stage_blocks):
            ...
            if 'Attn' in self.norm_cfg_ds['type']:
                self.norm_cfg_ds['num_affine_trans'] = self.num_affine_trans[i]
            if 'Attn' in self.norm_cfg1['type']:
                self.norm_cfg1['num_affine_trans'] = self.num_affine_trans[i]
            if 'Attn' in self.norm_cfg2['type']:
                self.norm_cfg2['num_affine_trans'] = self.num_affine_trans[i]
            if 'Attn' in self.norm_cfg3['type']:
                self.norm_cfg3['num_affine_trans'] = self.num_affine_trans[i]
        ...
        self.with_classification = self.num_classes > 0
        ...
        if self.deep_stem:
            self.stem = nn.Sequential(
                build_conv_layer(
                    ...
                    padding=1,
                    bias=False),
                build_norm_layer(self.norm_cfg_stem, stem_channels // 2)[1],
                ...
                build_conv_layer(
                    ...
                    padding=1,
                    bias=False),
                build_norm_layer(self.norm_cfg_stem, stem_channels)[1],
                nn.ReLU(inplace=True))

            # be consistent with impl. used in aognet-v2
            ...
            self.norm1_name, norm1 = build_norm_layer(
                self.norm_cfg_stem, stem_channels, postfix=1)
            self.add_module(self.norm1_name, norm1)
            self.relu = nn.ReLU(inplace=True)
            ...
            if self.zero_init_residual:
                for m in self.modules():
                    if isinstance(m, Bottleneck):
                        ...
                        if isinstance(m.norm2,
                                      (AttnBatchNorm2d, AttnGroupNorm)):
                            nn.init.constant_(m.norm2.weight_, 0.)
                            nn.init.constant_(m.norm2.bias_, 0.)
                        else:
    ...
    def forward(self, x):
        if self.with_hog and self.stage_with_hog[0]:
            with torch.no_grad():
                resize_x = nn.functional.interpolate(
                    x, scale_factor=2, mode='bilinear', align_corners=True)
            hog = self.hog0(resize_x)
            x = torch.cat((x, hog), dim=1)
    ...
    def train(self, mode=True):
        super(ResNetAN, self).train(mode)
        self._freeze_stages()
...
class ResNetV1dAN(ResNetAN):
    ...
    def __init__(self, **kwargs):
        super(ResNetV1dAN, self).__init__(
